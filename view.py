...
#Prints a message when connected to Discord.
@client.event
async def on_ready():
...
#Create a new Incident and submit it to Service Manager Server in the channel given as argument. It gathers and validates all the data and then send it to SM
...
#Update an Incident's fields. The communication with the user is done through a Discord channel given as argument.
...
async def updateIncident(channel):
    try:
        ...
        finished = False
        while not finished:
            #First the user is asked for the Incident ID. Already closed incidents cannot be modified
...
#Sends an error message to the channel given as argument.
